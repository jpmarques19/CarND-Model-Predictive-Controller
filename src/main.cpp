    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // These transforms make the job easier
          ...
          //converts vectors to Eigen
          double* ptrx = &ptsx[0];
          Eigen::Map<Eigen::VectorXd> ptsx_transform(ptrx, 6);
          double* ptry = &ptsy[0];
          Eigen::Map<Eigen::VectorXd> ptsy_transform(ptry, 6);

          //gets 3rd order coefficients for the line
          ...
          // cte and epsi before delay
          double cte0 = polyeval(coeffs, 0);
          double epsi0 = -delta;
          // cte and epsi after delay
          double cte_d = cte0 + ( v * sin(epsi0) * delay );
          double epsi_d = epsi0 - ( v * delta * delay / 2.67 );
          
        
          // state, after delay
          double x_d = v * cos(0) * delay;
          double y_d= v * sin(0) * delay;
          double psi_d = -(v * delta * delay / 2.67);
